...
typedef struct lucis_owned_node {
    void* ptr;
    struct lucis_owned_node* next;
} lucis_owned_node;

static void* (*lucis_sys_malloc)(size_t) = malloc;
static void* (*lucis_sys_realloc)(void*, size_t) = realloc;
static void  (*lucis_sys_free)(void*) = free;
static lucis_owned_node* lucis_owned_head = NULL;

static void lucis_owned_track_ptr(void* ptr) {
    if (!ptr) return;
    lucis_owned_node* node = (lucis_owned_node*)lucis_sys_malloc(sizeof(lucis_owned_node));
    if (!node) return;
    node->ptr = ptr;
    node->next = lucis_owned_head;
    lucis_owned_head = node;
}

static int lucis_owned_untrack_ptr(void* ptr) {
    lucis_owned_node** cur = &lucis_owned_head;
    while (*cur) {
        if ((*cur)->ptr == ptr) {
            lucis_owned_node* dead = *cur;
            *cur = dead->next;
            lucis_sys_free(dead);
            return 1;
        }
...
static void* lucis_tracked_malloc(size_t size) {
    void* ptr = lucis_sys_malloc(size);
    lucis_owned_track_ptr(ptr);
    return ptr;
}

static void* lucis_tracked_realloc(void* old_ptr, size_t size) {
    if (!old_ptr) return lucis_tracked_malloc(size);
    int was_owned = lucis_owned_untrack_ptr(old_ptr);
    void* new_ptr = lucis_sys_realloc(old_ptr, size);
    if (!new_ptr && was_owned) {
        lucis_owned_track_ptr(old_ptr);
        return NULL;
    }
    if (was_owned) lucis_owned_track_ptr(new_ptr);
    return new_ptr;
}

static void lucis_tracked_free(void* ptr) {
    if (!ptr) return;
    if (!lucis_owned_untrack_ptr(ptr)) return;
    lucis_sys_free(ptr);
}

#define malloc(sz) lucis_tracked_malloc(sz)
#define realloc(p, sz) lucis_tracked_realloc(p, sz)
#define free(p) lucis_tracked_free(p)
...
lucis_str_result lucis_toUpper(const char* s, size_t sLen) {
    ...
    buf[sLen] = '\0';
    return (lucis_str_result){ buf, sLen };
}
...
lucis_str_result lucis_trim(const char* s, size_t sLen) {
    ...
    buf[newLen] = '\0';
    return (lucis_str_result){ buf, newLen };
}
...
lucis_str_result lucis_trimRight(const char* s, size_t sLen) {
    ...
    memcpy(buf, s, end);
    buf[end] = '\0';
    return (lucis_str_result){ buf, end };
}
...
lucis_str_result lucis_padLeft(const char* s, size_t sLen,
    ...
    buf[width] = '\0';
    return (lucis_str_result){ buf, width };
}
...
lucis_str_result lucis_substring(const char* s, size_t sLen,
    ...
    buf[length] = '\0';
    return (lucis_str_result){ buf, length };
}
...
lucis_str_result lucis_joinVec(const lucis_str_vec_header* vec,
    ...
    return (lucis_str_result){ buf, total };
}
...
lucis_str_result lucis_fromCharsVec(const lucis_str_vec_header* vec) {
    ...
    return (lucis_str_result){ buf, count };
}
...
lucis_str_result lucis_fromCStr(const char* cstr) {
    if (!cstr) return (lucis_str_result){ "", 0 };
    size_t len = strlen(cstr);
    return (lucis_str_result){ cstr, len };
}
...
lucis_str_result lucis_fromCStrCopy(const char* cstr) {
    ...
    if (!buf) return (lucis_str_result){ "", 0 };
    memcpy(buf, cstr, len);
    buf[len] = '\0';
    return (lucis_str_result){ buf, len };
}
...
lucis_str_result lucis_capitalize(const char* s, size_t sLen) {
    if (sLen == 0) return (lucis_str_result){ "", 0 };
    char* buf = (char*)malloc(sLen + 1);
    if (!buf) return (lucis_str_result){ "", 0 };
    memcpy(buf, s, sLen);
...
lucis_str_result lucis_strRemove(const char* s, size_t sLen,
    ...
    if (start >= sLen) return (lucis_str_result){ s, sLen };
...
void lucis_words(lucis_str_vec_header* out, const char* s, size_t sLen) {
    size_t cap = 8;
    lucis_str_result* arr = (lucis_str_result*)malloc(cap * sizeof(lucis_str_result));
    size_t count = 0;
    ...
    while (i < sLen) {
        ...
        if (count >= cap) {
            cap *= 2;
            arr = (lucis_str_result*)realloc(arr, cap * sizeof(lucis_str_result));
        }
        arr[count++] = (lucis_str_result){ word, wordLen };
    }
